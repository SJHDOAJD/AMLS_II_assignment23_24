...
class CFG:
    ...
    seed=1110  # random seed
    target_size=5  # number of classes


def preprocess_ResNext():
    ...
    # get the processed train, valid, test data.
...
def train_ResNext(model,train_dataset,valid_dataset):
    # load data and give init values
    init_path = load_path()
    acc_train = 0
    acc_valid = 0
    ls_train = np.Inf
    ...
    # save diagram
    plot = os.path.join(init_path, 'plot', 'ResNext accuracy and loss diagram')
    plt.savefig(plot)
    plt.close()
...
def evaluate_ResNext(model, test_dataset):
    # load path and set parameter
    init_path = load_path()
    criterion = nn.CrossEntropyLoss()
    device = torch.device("cpu")
    # make test loader
    ...
    # save diagram
    plot = os.path.join(init_path, 'plot', 'ResNext confusion matrix diagram')
    plt.savefig(plot)
    plt.close()
    ...
    ax1.grid(True)
    # save diagram
    plot1 = os.path.join(init_path, 'plot', 'ResNext Performance Metrics diagram')
    plt.savefig(plot1)
    plt.close()
